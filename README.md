...
- boss直聘([Boss.java](backend/src/main/java/boss/Boss.java))【喜大普奔，目前Boss打招呼上限已修改为每日150次】
...
- 猎聘([Liepin.java](backend/src/main/java/liepin/Liepin.java))【默认打招呼无上限，主动发消息有上限，成功率不高，好在量大，较为推荐】

  > 注意：需要在猎聘App最新版设置打招呼语(默或者自定义皆可)，即可自动发送消息，不会被限制
  > 只可微信扫码，请绑定微信账号
  > 需要使用最新版猎聘手机app设置打招呼文本，只要不主动发消息，可以无限制对猎头打招呼，程序默认为该配置。

- 51job([Job.java](backend/src/main/java/job51/Job51.java))【投递有上限，且限制搜索到的岗位数量，没什么活人】

  > 51job现在已经烂掉了，不建议使用
  > 现在投递一段时间后会出现投递上限
  > 目前的解决方式是投一页暂停10秒，先这么着吧

- 智联招聘([ZhiLian.java](backend/src/main/java/zhilian/ZhiLian.java))【投递上限100左右，烂掉了，不要用】
...
- 🏃🏻‍♂️‍➡️ 运行启动类即可[GetJobsApplication.java](backend/src/main/java/com/getjobs/GetJobsApplication.java)
...
- ![img.png](backend/src/main/resources/img.png)
