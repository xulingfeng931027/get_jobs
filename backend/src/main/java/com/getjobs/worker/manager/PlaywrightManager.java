...
public class PlaywrightManager {
    ...
    // Playwright调试端口
    private static final int CDP_PORT = 7878;
    // 平台URL常量
    ...
    private final com.getjobs.worker.utils.PagePool pagePool =
            new com.getjobs.worker.utils.PagePool(4, 300000);
    // Playwright实例
    ...
    // Boss直聘平台是否被屏蔽（暂时不可用）
    private volatile boolean bossDisabled = true;
    ...
    // ========== 新增：配置参数注入 ==========
    @org.springframework.beans.factory.annotation.Value("${playwright.slow-mo:20}")
    private int slowMo;
    @org.springframework.beans.factory.annotation.Value("${playwright.navigation-timeout:30000}")
    private int navigationTimeout;
    @org.springframework.beans.factory.annotation.Value("${playwright.platforms.boss.navigation-timeout:30000}")
    private int bossNavigationTimeout;
    @org.springframework.beans.factory.annotation.Value("${playwright.platforms.liepin.navigation-timeout:35000}")
    private int liepinNavigationTimeout;
    @org.springframework.beans.factory.annotation.Value("${playwright.platforms.job51.navigation-timeout:40000}")
    private int job51NavigationTimeout;
    @org.springframework.beans.factory.annotation.Value("${playwright.platforms.zhilian.navigation-timeout:35000}")
    private int zhilianNavigationTimeout;
    @org.springframework.beans.factory.annotation.Value("${playwright.resource-blocking-enabled:false}")
    private boolean resourceBlockingEnabled;
    // ========== 新增：工具类注入 ==========
    ...
    public void init() {
        ...
        try {
            ...
            if (!chromiumFile.exists()) {
                ...
                log.error("请执行: npx playwright install chromium");
                throw new RuntimeException("Chromium 未安装或路径错误: " + chromiumPath);
            }
            log.info("✓ 找到 Chromium: {}", chromiumPath);

            browser = playwright.chromium().launch(new BrowserType.LaunchOptions()
                    .setHeadless(false) // 非无头模式,可视化调试
                    .setSlowMo(slowMo) // 放慢操作速度(可配置,默认20ms)
                    .setTimeout(60000) // 设置启动超时时间为60秒
                    .setArgs(List.of(
                            "--disable-setuid-sandbox",
                            "--disable-dev-shm-usage", // 禁用/dev/shm,避免内存问题
                            "--remote-debugging-port=" + CDP_PORT, // 使用固定CDP端口
                            "--start-maximized", // 最大化启动窗口
                            "--disable-extensions", // 禁用扩展,减少冲突
                            "--disable-software-rasterizer" // 禁用软件渲染器
                    )));
            log.info("✓ Chrome浏览器已启动 (调试端口: {}, slowMo: {}ms)", CDP_PORT, slowMo);
            ...
            if (resourceBlockingEnabled) {
                log.info("启用资源拦截器...");
                com.getjobs.worker.utils.ResourceBlocker.setupBlocking(context);
                log.info(com.getjobs.worker.utils.ResourceBlocker.getConfigSummary());
            }

            // 顺序创建所有Page（避免并发创建Page导致的竞态条件）
            log.info("开始创建所有平台的Page...");
//            bossPage = context.newPage();
//            bossPage.setDefaultTimeout(bossNavigationTimeout);
            log.info("✓ Boss Page已创建 (导航超时: {}ms)", bossNavigationTimeout);

            liepinPage = context.newPage();
            liepinPage.setDefaultTimeout(liepinNavigationTimeout);
            log.info("✓ 猎聘 Page已创建 (导航超时: {}ms)", liepinNavigationTimeout);

            job51Page = context.newPage();
            job51Page.setDefaultTimeout(job51NavigationTimeout);
            log.info("✓ 51job Page已创建 (导航超时: {}ms)", job51NavigationTimeout);

            zhilianPage = context.newPage();
            zhilianPage.setDefaultTimeout(zhilianNavigationTimeout);
            log.info("✓ 智联招聘 Page已创建 (导航超时: {}ms)", zhilianNavigationTimeout);

            // 注册到 PagePool 资源池
            pagePool.register("boss", bossPage);
            pagePool.register("liepin", liepinPage);
            pagePool.register("job51", job51Page);
            pagePool.register("zhilian", zhilianPage);
            log.info("✓ 所有 Page 已注册到资源池");
            ...
            // 初始化 CookieManager（从数据库加载记录）
            if (cookieManager != null) {
                try {
                    cookieManager.initFromDatabase("boss");
                    cookieManager.initFromDatabase("liepin");
                    cookieManager.initFromDatabase("job51");
                    cookieManager.initFromDatabase("zhilian");
                    log.info("✓ CookieManager 已初始化");
                } catch (Exception e) {
                    log.warn("CookieManager 初始化失败: {}", e.getMessage());
                }
            }

            log.info("✓ 浏览器自动化引擎初始化完成（所有平台已并发启动）");
            log.info(pagePool.getPoolStats());
            log.info("========================================");
        } catch (Exception e) {
     ...
     * 注意：Boss直聘已暂时屏蔽，仅标记为未登录状态
     */
    private void setupBossPlatform() {
        log.info("Boss直聘平台已暂时屏蔽，跳过初始化...");
        // 设置为未登录状态
        setLoginStatus("boss", false);
        // Boss页面跳转到空白页
        try {
            if (bossPage != null) {
                bossPage.navigate("about:blank");
            }
        } catch (Exception e) {
            log.debug("Boss页面屏蔽跳转失败: {}", e.getMessage());
        }
    }
    ...
    private void setupLiepinPlatform() {
        ...
        try {
            CookieEntity cookieEntity = cookieService.getCookieByPlatformAndUserId("liepin", currentUserId);
            if (cookieEntity != null && cookieEntity.getCookieValue() != null && !cookieEntity.getCookieValue().isBlank()) {
                ...
                if (!cookies.isEmpty()) {
                    ...
                    log.info("已从数据库加载猎聘 Cookie并注入浏览器上下文，用户={}，共 {} 条", currentUserId, cookies.size());
                } else {
    ...
    private void setup51jobPlatform() {
        ...
        try {
            CookieEntity cookieEntity = cookieService.getCookieByPlatformAndUserId("51job", currentUserId);
            if (cookieEntity != null && cookieEntity.getCookieValue() != null && !cookieEntity.getCookieValue().isBlank()) {
                String cookieStr = cookieEntity.getCookieValue();
                ...
                if (!cookies.isEmpty()) {
                    context.addCookies(cookies);
                    log.info("已从数据库加载51job Cookie并注入浏览器上下文，用户={}，共 {} 条", currentUserId, cookies.size());
                } else {
    ...
    private void save51jobCookiesToDatabase(String remark) {
        try {
            ...
            boolean result = cookieService.saveOrUpdateCookieWithUserId("51job", currentUserId, cookieJson, remark);
            if (result) {
                ...
                if (shouldInfoLog) {
                    log.info("保存51job Cookie成功，用户={}，共 {} 条，remark={}", currentUserId, cookies.size(), remark);
                    last51CookieLogMs = now;
                    last51CookieLogCount = cookies.size();
                    last51CookieRemark = remark == null ? "" : remark;
                } else {
                    // 近似重复的频繁调用，改为debug降低噪音
                    log.debug("保存51job Cookie成功(节流)，用户={}，条数={}，remark={}", currentUserId, cookies.size(), remark);
                }
            }
        } catch (Exception e) {
    ...
    private void setupZhilianPlatform() {
        ...
        try {
            CookieEntity cookieEntity = cookieService.getCookieByPlatformAndUserId("zhilian", currentUserId);
            if (cookieEntity != null && cookieEntity.getCookieValue() != null && !cookieEntity.getCookieValue().isBlank()) {
                String cookieStr = cookieEntity.getCookieValue();
                ...
                if (!cookies.isEmpty()) {
                    context.addCookies(cookies);
                    log.info("已从数据库加载智联招聘 Cookie并注入浏览器上下文，用户={}，共 {} 条", currentUserId, cookies.size());
                } else {
...
//                        log.warn("智联招聘：打开二维码登录面板失败: {}", e.getMessage());
    ...
    private void saveZhilianCookiesToDatabase(String remark) {
        try {
            ...
            // 使用ObjectMapper序列化为JSON字符串
            String cookieJson = new ObjectMapper().writeValueAsString(cookies);
            boolean result = cookieService.saveOrUpdateCookieWithUserId("zhilian", currentUserId, cookieJson, remark);
            if (result) {
                log.info("保存智联招聘Cookie成功，用户={}，共 {} 条，remark={}", currentUserId, cookies.size(), remark);
            }
        } catch (Exception e) {
    ...
    private void saveLiepinCookiesToDatabase(String remark) {
        try {
            ...
            // 使用ObjectMapper序列化为JSON字符串
            String cookieJson = new ObjectMapper().writeValueAsString(cookies);
            boolean result = cookieService.saveOrUpdateCookieWithUserId("liepin", currentUserId, cookieJson, remark);
            if (result) {
                log.info("保存猎聘Cookie成功，用户={}，共 {} 条，remark={}", currentUserId, cookies.size(), remark);
            }
        } catch (Exception e) {
...
//                isLoggedIn = checkIfLoggedIn();
    ...
    private void saveBossCookiesToDatabase(String remark) {
        try {
            ...
            // 使用ObjectMapper序列化为JSON字符串
            String cookieJson = new ObjectMapper().writeValueAsString(cookies);
            boolean result = cookieService.saveOrUpdateCookieWithUserId("boss", currentUserId, cookieJson, remark);
            if (result) {
                log.info("保存Boss Cookie成功，用户={}，共 {} 条，remark={}", currentUserId, cookies.size(), remark);
            }
        } catch (Exception e) {
    ...
    /**
     * 定时检查登录状态（每60秒兜底检查）
     * 主要依赖事件驱动（onFrameNavigated + onLoad）
     * 定时检查仅作为兜底，检测 Cookie 过期等场景
     */
    @Scheduled(fixedDelay = 60000)
    public void scheduledLoginCheck() {
        try {
            log.debug("定时兜底登录检查（60s间隔）");
    ...
    public void forceClose() {
        ...
        try {
            ...
            log.warn("强制关闭过程中发生异常: {}", e.getMessage());
        }
    }
     ...
     * @return 资源池统计字符串
     */
    public String getPagePoolStats() {
        return pagePool.getPoolStats();
    }
    ...
    public void setLoginStatus(String platform, boolean isLoggedIn) {
        ...
        if (previousStatus == null || previousStatus != isLoggedIn) {
            ...
            // 但如果Boss已被屏蔽，则跳过登录引导
            if ("boss".equals(platform) && !isLoggedIn && !bossDisabled) {
                try {
                    if (bossPage != null) {
                        String currentUrl = null;
                        try {
                            currentUrl = bossPage.url();
                        } catch (Exception ignored) {
                        }
